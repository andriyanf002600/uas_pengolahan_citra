...
def home_page():
    ...
    st.markdown(
        ...
        Aplikasi ini menggunakan dua teknologi yang bisa dipilih <strong>YOLOv8 dan EfficientNet-B7</strong> untuk mendeteksi penyakit pada daun mangga.
...
model2 = EfficientNetB0(weights="imagenet")  # Model kedua
...
def detection_page():
    ...
    tab1, tab2 = st.tabs(['\ud83d\udcc2 Model YOLOv8: Upload Gambar', '\ud83d\udcc2 Model EfficientNet-B7: Upload Gambar'])
    ...
    with tab2:
        ...
        if uploaded_image_model2:
            ...
            results = prediction_with_model(image, confidence, model2)
...
def view_results_page():
    ...
    for image_id, img in images:
        st.image(img, caption=f"Hasil Deteksi #{image_id}", use_container_width=True)
...
# Sidebar Navigasi
page = st.sidebar.radio("⚙️ Main Menu", ["🏠 Home", "🔍 Operasi Deteksi", "📊 Hasil Deteksi", "📈 Statistik"])
